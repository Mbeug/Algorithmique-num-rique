...
public class Utilities {
	 public static Matrix minus(Matrix a, Matrix b) {
	    ...
	    public static Matrix multplyByBand(Matrix base, Vector  diagA, Vector diagB){
	    	SimpleMatrix ret = new SimpleMatrix(base.getHeight(),base.getWidth());
			for (int i = 0; i < base.getHeight(); i++) {
				double tmp = base.get(i,0) * diagA.get(0);
				ret.set(tmp,i,0);
				for (int j = 1; j < base.getWidth(); j++) {
					tmp= base.get(i,j)*diagA.get(j) + base.get(i,j-1)*diagB.get(j-1);
					ret.set(tmp,i,j);
				}
			}
			return ret;
